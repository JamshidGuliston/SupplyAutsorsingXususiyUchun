<table>
	<tr>
		<td colspan="4"><b>Sana: "___".<?php printf('%02d', $days[0]->month_id % 12) ?>.
				<?php printf('%02d', $costs[0]->year_name) ?> / {{ $kindgar->kingar_name . " / " . $age->age_name }}</b>
		</td>
	</tr>
	<tr>
...
$costsumm = 0;
	...
	@foreach($nakproducts as $key => $row)
		<tr>
			...
			<td><?php $costsumm += $summ * $row[0];
		printf("%01.2f", $summ * $row[0]) ?></td>
			...
			<td><?php printf("%01.2f", $summ * $row[0] + ($summ * $row[0] * $ust) / 100 + (($summ * $row[0] + ($summ * $row[0] * $ust) / 100) * $nds) / 100) ?>
			</td>
		</tr>
	@endforeach
	<tr>
	<tr>
		...
		<td><?php printf("%01.3f", $costsumm + ($costsumm * $ust) / 100 + ($costsumm + ($costsumm * $ust) / 100) * $nds / 100); ?>
		</td>
	</tr>
</table>